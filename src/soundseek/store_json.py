"""JSON file persistence for setlists (local/dev/CLI backend).
...
def save(setlist: Setlist) -> None:
    ...
    setlist_path(setlist.id).write_text(setlist.model_dump_json(indent=2), encoding="utf-8")
    index[setlist.source_url] = setlist.id
    _write_index(index)


def load(setlist_id: str) -> Setlist:
    return Setlist.model_validate_json(setlist_path(setlist_id).read_text(encoding="utf-8"))
